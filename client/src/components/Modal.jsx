...
const ModalComentario = ({ isOpen, onClose, onSubmit, student }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50">
      <div className="bg-white p-6 rounded-lg bg-white-1 w-[28.625rem] h-[18.9rem] border border-brand-primary">
        <div className="flex justify-end ml-2">
          <button onClick={onClose} className="text-gray-700 rounded ">
        ...
        <div>
          <ul className="mx-auto ">
            ...
            <li className="text-gray-400 text-sm">{student?.courses}</li>
            <li className="text-gray-400 text-sm">{student?.grado}</li>
          </ul>
          <textarea
            value={comentarios}
            onChange={(e) => setComentarios(e.target.value)}
            placeholder="Escriba comentarios sobre el desempeño del estudiante en este período académico..."
            className="p-2 border-2 border-[#9E9E9E] rounded w-[25rem] h-[70px] text-sm  block text-gray-700 focus:outline-none overflow-hidden resize-none mt-4"
            rows="4"
            maxLength={500}
          />
          <div className="flex justify-between mt-2">
          ...
          <div className="flex justify-end mt-4">
            <button
              onClick={handleSubmit}
              className="flex items-center justify-center  bg-brand-primary text-white-1 rounded w-[86px] h-[36px]"
            >
              Guardar
            </button>
          </div>
